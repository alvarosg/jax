...
def map_coordinates(
  ...
  Args:
    ...
    mode: Points outside the boundaries of the input are filled according to the given mode.
      ...
      `'wrap'` mode in JAX behaves as '`grid-wrap'` mode in SciPy, and `'constant'` mode 
      in JAX behaves as '`grid-constant'` mode in SciPy. This discrepancy was caused by 
      a former bug in those modes in SciPy (https://github.com/scipy/scipy/issues/2640), 
